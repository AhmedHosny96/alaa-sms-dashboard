...
const RANGE_COLUMNS = [
  { accessorKey: 'range', header: 'Range', meta: { headerProps: { className: 'text-900' } } },
  { accessorKey: 'totalNumbers', header: 'Total Numbers', meta: { headerProps: { className: 'text-900' } } },
  { accessorKey: 'active', header: 'Active', meta: { headerProps: { className: 'text-900' } } },
  { accessorKey: 'smsReceived', header: 'SMS Received', meta: { headerProps: { className: 'text-900' } } },
  {
...
// Dimensions available for client-side re-aggregation. Each must be a key
// already present on the API row (no backend changes required).
...
const RangeStats = () => {
  ...
  const fetchStats = useCallback(async (silent = false) => {
    ...
    try {
      ...
      const res = await smsService.getRangeStats(params);
  ...
  const filteredData = useMemo(() => {
    ...
    if (query) {
      ...
      list = list.filter((row) =>
        [row.range, row.totalNumbers, row.active, row.smsReceived, row.utilization]
          .filter((v) => v != null)
  ...
  // Client-side re-aggregation by the chosen dimension. Mirrors CdrReports'
  // group-by: collapse rows by group value, sum numeric counters.
  const groupedData = useMemo(() => {
    ...
    filteredData.forEach((row) => {
      ...
      item.totalNumbers += toNumber(row?.totalNumbers);
      item.active += toNumber(row?.active);
    ...
    return Array.from(map.values()).map((item) => ({
      ...item,
      utilization: item.totalNumbers > 0
        ? `${((item.active * 100) / item.totalNumbers).toFixed(2)}%`
        : '0.00%',
  ...
  const groupedColumns = useMemo(() => {
    if (!groupByKey) return RANGE_COLUMNS;
    const groupDef = GROUP_BY_OPTIONS.find((g) => g.key === groupByKey);
    return [
      { accessorKey: 'groupValue', header: groupDef?.label || 'Group', meta: { headerProps: { className: 'text-900' } } },
      { accessorKey: '_rangeCount', header: 'Ranges', meta: { headerProps: { className: 'text-900' } } },
      { accessorKey: 'totalNumbers', header: 'Total Numbers', meta: { headerProps: { className: 'text-900' } } },
      { accessorKey: 'active', header: 'Active', meta: { headerProps: { className: 'text-900' } } },
      { accessorKey: 'smsReceived', header: 'SMS Received', meta: { headerProps: { className: 'text-900' } } },
      { accessorKey: 'utilization', header: 'Utilization', meta: { headerProps: { className: 'text-900' } } },
  ...
  const activeColumns = groupByKey ? groupedColumns : RANGE_COLUMNS;
  const activeGroupLabel = GROUP_BY_OPTIONS.find((g) => g.key === groupByKey)?.label || '';

  const summary = useMemo(() => {
    const totals = { totalNumbers: 0, active: 0, smsReceived: 0, myPayout: 0, agentPayout: 0 };
    filteredData.forEach((row) => {
      totals.totalNumbers += toNumber(row?.totalNumbers);
      totals.active += toNumber(row?.active);
  ...
  const exportColumns = useMemo(
    () => groupByKey
      ? [activeGroupLabel || 'Group', 'Ranges', 'Total Numbers', 'Active', 'SMS Received', 'Utilization', 'My Payout', 'Agent Payout']
      : ['Range', 'Total Numbers', 'Active', 'SMS Received', 'Utilization', 'My Payout', 'Agent Payout', 'Profit %'],
      ...
      table.getSortedRowModel().rows.map((r) => {
        ...
        if (groupByKey) {
          return {
            ...
            Ranges: row._rangeCount ?? '-',
            'Total Numbers': row.totalNumbers ?? '-',
            Active: row.active ?? '-',
            'SMS Received': row.smsReceived ?? '-',
          ...
          <Card.Header>
            <Row className="flex-between-center">
              <Col xs={4} sm="auto" className="d-flex align-items-center pe-0">
                <h5 className="fs-9 mb-0 text-nowrap py-2 py-xl-0">SMS Range Stats</h5>
                  ...
                  >
                    ...
                    <TableExportSelect
                      ...
                      onExport={(type) =>
                        exportRowsByType({
                          type,
                          title: 'SMS Range Stats',
                          filenamePrefix: 'sms-range-stats',
            ...
            <div className="border-top bg-white px-3 py-3">
              ...
              >
                <div className="text-center">
                  <div className="text-uppercase text-600 fw-bold mb-2">Total Numbers</div>
                  <div className="fw-bold fs-9">{summary.totalNumbers.toLocaleString()}</div>
                </div>
                <div className="text-center">
                  <div className="text-uppercase text-600 fw-bold mb-2">Active</div>
                  <div className="fw-bold fs-9">{summary.active.toLocaleString()}</div>
                </div>
                <div className="text-center">
                  <div className="text-uppercase text-600 fw-bold mb-2">SMS Received</div>
