...
const RANGE_COLUMNS = [
  ...
  { title: 'Total Numbers', dataIndex: 'totalNumbers', key: 'totalNumbers' },
  { title: 'Active', dataIndex: 'active', key: 'active' },
  { title: 'SMS Sent', dataIndex: 'smsSent', key: 'smsSent' },
  { title: 'SMS Received', dataIndex: 'smsReceived', key: 'smsReceived' },
  { title: 'Utilization', dataIndex: 'utilization', key: 'utilization' }
];
...
const RangeStats = () => {
  ...
  const columns = useMemo(() => RANGE_COLUMNS, []);
  ...
  const filteredData = useMemo(() => {
    ...
    if (query) {
      ...
      list = list.filter((row) =>
        [row.range, row.totalNumbers, row.active, row.smsSent, row.smsReceived, row.utilization]
          .filter((v) => v != null)
  ...
  return (
    <TablePageLayout
      ...
      subtitle="Statistics for SMS number ranges."
      toolbar={
        <>
    ...
    >
      <TableContainer dataSource={filteredData} loading={loading} rowKey={(r) => r.id ?? r.range} />
    </TablePageLayout>
  );
};
